...
class Planetoids(GameApp):
    ...
    # ADD MORE ATTRIBUTES
    # Attribute _lastkeys: count of key pressed in last frame
    # Invariant: int
    ...
    def start(self):
        ...
        self._state = STATE_INACTIVE       # game starts as INACTIVE state
        
        if self._state == STATE_INACTIVE:
            self._title = write('title', 'Planetoids')
            self._message = write('message', "Press 'S' to Start")
        ...
        # # for state change we have to keep track on key is pressed or not
        # self._lastkeys = 0
    ...
    def update(self,dt):
        ...
        if self._state == STATE_LOADING:
            ...
            self._message = write('message',"Press 'S' to Continue")
            self._wave.frame_count = 0
            ...
            self._message = write('message',"Game Over\n Thankyou! ")
    ...
    def draw(self):
        ...
        # IMPLEMENT ME
        # background color
        GLabel(text = '',height = GAME_HEIGHT*2, width = GAME_WIDTH*2, fillcolor = RGB(0,0,155)).draw(self.view)
    ...
    def _determineStates(self):
        
        '''
        
        '''
        var = self.input.is_key_down('s') and not self._sdown

        if var and self._state == STATE_INACTIVE:
            self._state = STATE_LOADING
        elif var and self._state == STATE_PAUSED:
            self._state = STATE_CONTINUE
        elif var and self._state == STATE_COMPLETE:
            self._state = STATE_LOADING

        elif self._state == STATE_ACTIVE and self._wave.is_asteroid_ended():
            self._state = STATE_COMPLETE
            
        self._sdown = self.input.is_key_down('s')

def write(form, text):     # helper function to create message and title
    '''
    returns instance of GLabel to write message and title for welcome screen
    Parameter form: format of instantiation i.e. message or title
    Precondition:   str
    '''
    if form == 'message':
        return GLabel(text = text, font_size = MESSAGE_SIZE, left = 150,
                        font_name = MESSAGE_FONT, linecolor = 'white',  bottom = 300)
    elif form == 'title':
        return GLabel(text = text, font_size = TITLE_SIZE, left = 50,
                        font_name = TITLE_FONT,linecolor = 'white', bottom = 400)