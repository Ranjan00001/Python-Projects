...
def draw_hex(t, s):
    ...
    # here we don't need to restore the color because triangle is restoring it
    for i in range(6):
...
def draw_spiral_helper(t, side, ang, n, sp):
    ...
    assert is_number(ang), report_error('ang is not a valid ang',ang)
...
def multi_polygons(w, side, k, n, sp):
    ...
    assert is_window(w), report_error('w is not a valid window',w)
    assert is_valid_length(side), report_error('side is not a valid length',side)
    assert is_valid_iteration(k), report_error('this is not valid', k)
    assert type(n) == int and n >= 3 , report_error('this is not an int', n)
    assert is_valid_speed(sp), report_error('sp is not a valid speed',sp)
...
def multi_polygons_helper(t, side, k, n, sp):
    """
    Draws k n-sided polygons of side length side.
...
def draw_polygon(t, side, n):
    ...
    assert is_valid_turtlemode(t), report_error('Invalid turtle mode', t)
    assert is_valid_length(side), report_error('side is not a valid length',side)
    assert is_valid_iteration(n), report_error('n is an invalid # of poly sides',n)
...
def radiate_petals(w, radius, width, n, sp):
    ...
    assert type(n) == int and n >= 2 , report_error('this is not an int', n)
    assert is_valid_speed(sp), report_error('sp is not a valid speed',sp)
...
def radiate_petals_helper(t, radius, width, n, sp):
    ...
    for i in range(n//2):
        # ang = ang+360/n
        t.color = introcs.HSV(t.heading % 360, 1, 1)
        t.forward(radius)
        t.backward(radius)
        t.left(360/n)
...
def vicsek(w, size, d, sp):
    ...
    assert type(d) == int and d >= 0, report_error('d is not a valid',d)
    assert is_valid_speed(sp), report_error('not a valid speed',sp)
...
def vicsek_helper(p, x, y, size, d):
    ...
    # ARE THESE ALL OF THE PRECONDITIONS?
    assert is_valid_penmode(p), report_error('Invalid pen mode', p)
    assert is_number(x), report_error('not a valid number', x)
    assert is_number(y), report_error('not a valid number', y)
    assert is_valid_length(size), report_error('size is not a valid length',size)
    assert type(d) == int and d >= 0, report_error('d is not a valid int',d)
    ...
    if d == 0:
        ...
        size = size/3
        vicsek_helper(p, x, y, size, d-1)
        vicsek_helper(p, x+size, y, size, d-1)
        vicsek_helper(p, x, y+size, size, d-1)
        vicsek_helper(p, x-size, y, size, d-1)
        vicsek_helper(p, x, y-size, size, d-1)
        
        # fill_rect(p, x, -(y+size), size, size)
        # fill_rect(p,(x+size), y, size, size)
        # fill_rect(p, x, (y+size), size, size)
        # fill_rect(p, -(x+size), y, size, size)
...
def branches(w, hght, ang, d, sp):
    ...
    Parameter ang: The branch angles (measured from the central branch)
    Precondition: angle is a number, 0 < ang < 180
    ...
    # ARE THESE ALL OF THE PRECONDITIONS?
    assert is_window(w), report_error('w is not a valid window',w)
    assert is_valid_length(hght), report_error('not a valid hght', hght)
    assert is_number(ang) and 0 < ang < 180, report_error('ang is not valid', ang)
    assert type(d) == int and d >= 0, report_error('d is not a valid',d)
    assert is_valid_speed(sp), report_error('sp is not valid', sp)
...
def branches_helper(t, hght, ang, d):
    ...
    #     initial_y = t.y
    #     t.forward(hght)
    #     t.move(initial_x,initial_y)
    #     t.heading = initial_heading

    # else:
    #     branches_helper(t, hght, 90, d-1)
    #     hght = hght/2
    #     t.move(t.x,hght)
    #     t.right(ang)
    #     branches_helper(t, hght, ang, d-1)
    #     t.left(ang)
    #     branches_helper(t, hght, ang, d-1)
    #     # hght = hght/2
    #     # t.forward(hght)
