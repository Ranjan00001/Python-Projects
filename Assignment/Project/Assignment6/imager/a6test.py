...
def compare_images(image1,image2,file1,file2):
    ...
    for col in range(image2.getWidth()):
        for row in range(image2.getHeight()):
            # print(image1.getPixel(row, col), image2.getPixel(row, col))
...
def test_decode():
    ...
    image = load_image('blocks')
    encoder = a6encode.Encoder(image)
    result = encoder.decode()   # here we haven't encoded anything so decoding it must return None
    introcs.assert_equals(None,result)
    
    encoder.increment()
    encoder.encode('')         # here we have encoded '' so decoding it must return ''
