...
def _is_pixel_list(data):
    ...
    for i in data:
        if not _is_pixel(i):     # checks each element is a pixel or not
            return False
    return True
...
class Image(object):
    ...
    The methods `__getitem__` and `__setitem__` provide operator overloading for [].
    ...
    def getData(self):
        ...
        """
        return self._data[:]     # have to return the copy
    ...
    def setWidth(self,value):
        ...
        """
        assert isinstance(value, int) and value >= 0, repr(value)+' is not a valid width'
        assert len(self._data) % value == 0, repr(value)+' is not valid value'

        self._width = value    # first set the given value to width
        if value != 0:
            self._height = len(self._data) // value    # changing height accordingly
        else:
            self._data = []    # for 0 width data list will be empty
    ...
    def setHeight(self,value):
        ...
        """
        assert isinstance(value, int) and value >= 0, repr(value)+' is not valid height'
        assert len(self) % value == 0, repr(value)+' is not valid value'
        
        self._height = value    # first set the given value to height
        if value != 0:
            self._width = len(self._data) // value   # changing width accordingly
        else:
            self._data = []   # for 0 height data list will be empty
    ...
    def __init__(self, data, width):
        ...
        """
        assert _is_pixel_list(data), repr(data)+' is not valid'
        self._data = data
        self.setWidth(width)
    ...
    def __getitem__(self, pos):
        ...
        """
        assert isinstance(pos, int), repr(pos)+' is not an int'
        assert pos < len(self)
    ...
    def getPixel(self, row, col):
        ...
        """
        assert isinstance(row, int) and self.getHeight() > row >= 0, repr(row)+' is not a valid row'
        assert isinstance(col, int) and  self.getWidth() > col >= 0, repr(col)+' is not a valid column'

        position = row * self.getWidth() + col
    ...
    def __str__(self):
        ...
        """
        lst_1d = self._data
        for i in range(0, len(self), self.getWidth()):
            # print(i, len(lst_1d), self.getWidth())
            # print(lst_1d[i : i + self.getWidth()])
            if i != 0:
                str_2d = str_2d + '\n' + str(lst_1d[i : i + self.getWidth()]) + ','
            else:
                str_2d = str(lst_1d[i : i + self.getWidth()]) + ','
                # print(str_2d)
        return '[' + str_2d[:-1] + ']'
